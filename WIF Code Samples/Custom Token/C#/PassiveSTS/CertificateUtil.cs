    ...
    public static X509Certificate2 GetCertificate( StoreName name, StoreLocation location, string subjectName )
    {
        ...
        try
        {
            ...
            for ( int i = 0; i < certificates.Count; i++ )
            {
                ...
                if ( cert.Thumbprint.ToUpper() == subjectName.ToUpper() )
                {
                    if ( result != null )
                    {
                        throw new ApplicationException( string.Format( "More than one certificate was found for thumbprint {0}", subjectName ) );
                    }
