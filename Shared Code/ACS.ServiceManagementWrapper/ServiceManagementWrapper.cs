...
namespace ACS.ServiceManagementWrapper
{
    ...
    public class ServiceManagementWrapper
    {
        ...
        private static DateTime GetExpiryTime(string swtToken)
        {
            ...
            var expiresOnUnixTicks = int.Parse(properties["ExpiresOn"], CultureInfo.InvariantCulture);
        ...
        private string GetTokenFromACS_old()
        {
            var client = new WebClient { BaseAddress = string.Format(CultureInfo.InvariantCulture, "https://{0}.{1}", this.serviceNamespace, Constants.AcsHostName) };

            var values = new NameValueCollection
                             {
                                 { "wrap_name", this.serviceIdentityUsernameForManagement }, 
                                 { "wrap_password", this.serviceIdentityPasswordForManagement }, 
                                 { "wrap_scope", string.Format(CultureInfo.InvariantCulture, "https://{0}.{1}/mgmt", this.serviceNamespace, Constants.AcsHostName) }
                             };
